...
POLICY_RECORD_KEYS = {"type", "config_path", "name"}
...
def resolve_policy_config_path(
    ...
    """
    policy = record_cfg.get("policy", record_cfg)
    policy_type = str(policy.get("type", "")).strip().lower()
    raw_path = policy.get("config_path") or _default_policy_config_path(policy_type)
...
def _normalization_mode_from_config(value: Any) -> NormalizationMode:
...
def _convert_normalization_mapping(value: Any) -> dict[str, NormalizationMode]:
...
def _feature_type_from_config(value: Any) -> FeatureType:
...
def _convert_policy_features(value: Any, field_name: str) -> dict[str, PolicyFeature]:
    if not isinstance(value, dict):
...
def _normalize_temporal_ensemble_coeff(value: Any) -> float | None:
...
def _restore_indexed_device_if_available(policy_cfg: PreTrainedConfig, indexed_device: str | None) -> None:
...
def build_act_config(
    ...
    allowed_fields = {config_field.name for config_field in fields(ACTConfig)}
    unknown_fields = sorted(set(policy_yaml_dict) - allowed_fields)
    if unknown_fields:
        raise ValueError(
            "Unknown ACT policy config field(s): "
            f"{unknown_fields}. Allowed fields are: {sorted(allowed_fields)}"
        )

    act_kwargs = dict(policy_yaml_dict)
    legacy_policy_dict = legacy_policy_dict or {}
    legacy_overrides = {
        key: value
        for key, value in legacy_policy_dict.items()
        if key not in POLICY_RECORD_KEYS and key in allowed_fields
    }
    if legacy_overrides:
        logging.warning(
            "Deprecated ACT policy field(s) found in record_cfg.yaml: %s. "
            "Policy hyperparameters have moved to scripts/policy_config/*.yaml. "
            "For backward compatibility, these legacy values override the policy yaml for this run.",
            sorted(legacy_overrides),
        )
        act_kwargs.update(legacy_overrides)
    ...
    if "normalization_mapping" in act_kwargs:
        act_kwargs["normalization_mapping"] = _convert_normalization_mapping(
            act_kwargs["normalization_mapping"]
        )

    if "temporal_ensemble_coeff" in act_kwargs:
        act_kwargs["temporal_ensemble_coeff"] = _normalize_temporal_ensemble_coeff(
...
def build_policy_config(
    ...
    if policy_type in ACT_POLICY_TYPES:
        return build_act_config(policy_yaml_dict, legacy_policy_dict=legacy_policy_dict)
    if policy_type in DIFFUSION_POLICY_TYPES:
