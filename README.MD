# Dual Arm Teleoperation and Data Collection System

## Introduction 
This project is a teleoperation data collection tool for dual arm robots, embeded in [LeRobot](https://github.com/huggingface/lerobot.git).
...
<p align="center">
  <img src="assets/arx.jpeg" alt="Record" width="600">
  <br>
</p>


It supports the end-effector pose control of the robot using Oculus Quest 3/3s devices, and collect the teleoperation data in LeRobot dataset format.
...
The Oculus Quest controller provides 6-DoF tracking for intuitive end-effector control. 
...
### 1.3 Clone and Install Dual Arm Teleoperation Source
```bash
mkdir dual_arm_ws && cd dual_arm_ws
...
```bash
python teleoperators/oculus_teleoperator/oculus/oculus_reader/oculus_reader/reader.py
```
to test the Oculus Reader and check the data output.

#### 2.2.1 Install ADB (Android Debug Bridge)
...
#### 2.2.2 Enable Developer Mode on Oculus Quest
...
#### 2.2.3 Connect Oculus Quest to Computer
...
#### 2.2.4 Install Oculus Reader APK
...
#### 2.2.5 Configure Oculus Teleoperation in record_cfg.yaml
...
#### 2.2.6 Oculus Controller Controls
...
#### 2.2.7 Coordinate System Mapping
...
## 4. Dataset Replay
```bash
robot-replay # Make sure record_cfg.yaml is properly configured
```

## 5. Dataset Visualization
```bash
robot-visualize # Make sure record_cfg.yaml is properly configured
```
<p align="center">
...
### 7.1 Dataset Preprocessing and Sub-Episode Labeling
Use these helper scripts when recorded episodes need cleanup, subtask segmentation, language labels, or VLA manifests before training.

1. `preprocess_dataset.py` rewrites a LeRobot dataset to a separate output dataset. It can trim long static spans, preserve context around gripper transitions, and optionally smooth or clamp delta end-effector action labels. Configure `scripts/config/preprocess_dataset_cfg.yaml`.
```bash
python scripts/tools/preprocess_dataset.py --config scripts/config/preprocess_dataset_cfg.yaml --dry-run --max-episodes 2
python scripts/tools/preprocess_dataset.py --config scripts/config/preprocess_dataset_cfg.yaml --overwrite
```

2. `split_label_dataset.py` splits long episodes into sub-episodes, labels each segment, writes `split_manifest.jsonl` and `vla_manifest.jsonl`, and can create a StarVLA-style `modality.json`. When semantic segmentation is enabled, it uses OpenRouter first and falls back to gripper/motion rules if configured. Configure `scripts/config/split_label_dataset_cfg.yaml`.
```bash
export OPENROUTER_API_KEY=<your_key>
python scripts/tools/split_label_dataset.py --config scripts/config/split_label_dataset_cfg.yaml --dry-run --max-episodes 1
python scripts/tools/split_label_dataset.py --config scripts/config/split_label_dataset_cfg.yaml --label-only
python scripts/tools/split_label_dataset.py --config scripts/config/split_label_dataset_cfg.yaml --overwrite
```

- Keep `source.root` and `output.root` different. Both scripts refuse in-place writes.
- For offline or template labeling, set `openrouter.enabled: false`; dry-runs do not upload images unless `semantic_segmentation.run_on_dry_run: true`.
- If `language_augmentation.write_variants_as_episodes: true`, one segment may become multiple output episodes with different task strings.
...
### 8.1 DAgger Full-Episode Success Policy

`full_success_episode` and `hybrid` exports with full episodes use
`full_episode.success_policy` in `dagger_rounds_cfg.yaml`:
...
When using `recorded_is_success`, the operator must be strict about deleting any episode that should not become a full demonstration. The success labels can later support VLA/DP/failure-detector filtering and evaluation.
...
4. Run `robot-record`. For safety, ensure the master and slave arm joint angles are approximately aligned; `robot-record` will automatically perform the `2.4 Master-Slave Joint Angle Error Check`. Then follow `8. Recording Control Keys` to complete the recording.
