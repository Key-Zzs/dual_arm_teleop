"""
Franka dual-arm robot implementation.
Each arm has 7 DOF with Robotiq 2F-85 gripper as end effector.
...
class FrankaDualArm(Robot):
    """
    Dual-arm Franka robot with two Robotiq 2F-85 grippers.
    ...
    def connect(self) -> None:
        ...
        logger.info("\n" + "=" * 60)
        logger.info("[ROBOT] Connecting to Franka Dual-Arm System")
        logger.info("=" * 60)
        
        # Connect to dual-arm server (single port)
        self._robot = self._check_franka_connection()
    ...
    def _check_franka_connection(self) -> FrankaDualArmClient:
        """Connect to Franka dual-arm server via zerorpc (single port)."""
        try:
            logger.info("\n===== [ROBOT] Connecting to Franka dual-arm =====")
            
            robot = FrankaDualArmClient(
            ...
            logger.info("===== [ROBOT] Franka dual-arm connected successfully =====\n")
            return robot
            
        except Exception as e:
            logger.error("===== [ERROR] Failed to connect to Franka dual-arm =====")
