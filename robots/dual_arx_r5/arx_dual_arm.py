"""
Arx dual-arm robot implementation.
Each arm has 6 DOF.
...
class ArxDualArm(Robot):
    ...
    def __init__(self, config: ArxDualArmConfig):
        ...
        self._robot: Optional[ArxDualArmClient] = None
    ...
    def connect(self) -> None:
        ...
        logger.info("\n" + "=" * 60)
        logger.info("[ROBOT] Connecting to Arx Dual-Arm System")
        logger.info("=" * 60)
        
        # Connect to dual-arm server (single port)
        self._robot = self._check_arx_connection()
    ...
    def _check_arx_connection(self) -> ArxDualArmClient:
        """Connect to Arx dual-arm server via zerorpc (single port)."""
        try:
            logger.info("\n===== [ROBOT] Connecting to Arx dual-arm =====")
            
            robot = ArxDualArmClient(
            ...
            logger.info("===== [ROBOT] Arx dual-arm connected successfully =====\n")
            return robot
            
        except Exception as e:
            logger.error("===== [ERROR] Failed to connect to Arx dual-arm =====")
