...
## 简介
本项目用于双臂系统的遥操作数据采集与回放，嵌入在 LeRobot 框架中。
...
### 2.2 Oculus Quest 设置（用于 Oculus 遥操作模式）

如果您计划使用 Oculus Quest 进行遥操作，请按照以下步骤设置设备：

#### 2.2.1 安装 ADB（Android 调试桥）
...
#### 2.2.5 在 record_cfg.yaml 中配置 Oculus 遥操作
...
## 4. 数据集回放
```bash
robot-replay #注意cfg配置
```

## 5. 数据集可视化
```bash
robot-visualize #注意cfg配置
```
<p align="center">
...
```bash
lerobot-edit-dataset --repo_id <merged_repo_id> --operation.type merge --operation.repo_ids "['<repo_id_1>', '<repo_id2>']"
```
- 更多数据集处理命令，请参考 [LeRobot](https://huggingface.co/docs/lerobot/using_dataset_tools)

### 7.1 数据预处理与子片段标注
如果录制好的 episode 在训练前需要清理静止段、拆分子任务、生成语言标签或导出 VLA manifest，可以使用下面两个工具脚本。

1. `preprocess_dataset.py` 会把源 LeRobot 数据集重写到一个新的输出数据集。它可以裁剪长时间无动作片段、保留夹爪开合前后的上下文，并可选地对 delta 末端位姿 action 做平滑或限幅。配置文件为 `scripts/config/preprocess_dataset_cfg.yaml`。
```bash
...
2. `split_label_dataset.py` 会把长 episode 拆成子 episode，为每个片段生成语言标签，写出 `split_manifest.jsonl`、`vla_manifest.jsonl`，并可生成 StarVLA 风格的 `modality.json`。开启语义分段时会优先调用 OpenRouter；如果配置允许，会回退到夹爪/运动规则分段。配置文件为 `scripts/config/split_label_dataset_cfg.yaml`。
```bash
...
- `source.root` 和 `output.root` 必须不同；两个脚本都会拒绝原地覆盖源数据集。
- 如需离线或模板标签，可设置 `openrouter.enabled: false`；`--dry-run` 默认不会上传图片，除非设置 `semantic_segmentation.run_on_dry_run: true`。
- 如果 `language_augmentation.write_variants_as_episodes: true`，一个子片段会按不同 task 文本写成多个输出 episode。
...
### 8.1 DAgger full-episode success 策略

`full_success_episode` 和包含 full episode 的 `hybrid` 导出会使用
`dagger_rounds_cfg.yaml` 中的 `full_episode.success_policy`：

- `explicit`：最严格。每条保存的 run_mix episode 结束后人工确认 success，适合需要显式成功/失败标签的数据。
- `recorded_is_success`：保存即成功。适合“失败、不完整、轨迹质量差的数据用左箭头删除”的当前工作流；保存下来的 episode 会写入 `success=true`，并标记为从已保存 episode 推断。
- `allow_missing_for_smoke`：仅用于调试。缺少 success 字段也允许导出，但会输出 warning，不建议正式训练。

使用 `recorded_is_success` 时，操作者必须严格删除失败、不完整、质量差或不适合作为完整示范的数据。保留下来的 success 标签后续可用于 VLA、DP、failure detector 的数据筛选与评估。
...
4. 运行命令 `robot-record`，为确保安全， 请保证主臂与从臂关节角近似，`robot-record` 会自动执行 `2.4 获取主臂-从臂关节角误差周期` 检查，然后按照 `8. 录制控制按键说明` 完成录制操作。
